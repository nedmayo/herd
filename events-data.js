(() => {
  ...
  const events = [
    {
      ...
      imageMode: "poster",
      imageSrc: "content/Herd Site Why Not.png",
      imageAlt: "HERD: Why Not?",
    ...
    },
    {
      ...
      detailsHtml:
        '<p><a href="pressure.html" class="event-read-more-inline">Read more</a></p>',
    },
    {
      id: "outside-2026",
      date: "2026-04-22",
  ...
  function renderIndexCard(event) {
    const indexImageHtml =
      event.imageMode === "logo"
        ? `
                <div class="event-image-placeholder">
  ...
  function renderEventsPage() {
    const upcomingGrid = document.querySelector(
      "#upcoming-events .events-cards-grid",
    );
    ...
    upcomingGrid.innerHTML = upcoming
      .map((event) => renderEventsPageCard(event, false))
      .join("");
    pastGrid.innerHTML = past
      .map((event) => renderEventsPageCard(event, true))
      .join("");
  }

  function renderIndexUpcomingPreview(maxItems = 4) {
    const previewContainer = document.getElementById(
      "index-upcoming-events-preview",
    );
